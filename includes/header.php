    ...
    <div
        ...
        <div class="flex gap-7">
            ...
            <a href="chatboard.php"
                ...
                <?php
                if (isset($userId)) {
                    $chatCountQuery = $connect->query("
                   SELECT COUNT(DISTINCT books.id) AS total_chats
                  FROM message
                   JOIN books ON message.product_id = books.id
                    WHERE message.sender_id = '$userId' OR message.receiver_id = '$userId'
                   ");

                    if ($chatCountQuery && $chatCountQuery->num_rows > 0) {
                        $chatCountRow = $chatCountQuery->fetch_assoc();
                        $chatCountData = $chatCountRow['total_chats'];

                          if ($chatCountData > 0) {
                  ?>
                            <div class="absolute -top-1 -right-1 inline-flex items-center justify-center w-5 h-5 text-xs font-bold text-white bg-red-500 border border-white rounded-full transform group-hover:scale-110 transition-transform">
                                <?= $chatCountData; ?>
                            </div>
                  <?php
                        }
