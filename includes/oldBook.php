...
<div class="px-[5%]">
        <div class="flex   border-b   border-gray-500 item-center  flex-1 justify-between items-center py-4 px-6x">
            <h1 class="font-bold text-2xl text-[#] ">Old Book</h1>
            <a href="#"
                class="px-6 py-3 bg-white text-[#3D8D7A] rounded-full font-medium transition-all duration-300 hover:bg-[#3D8D7A] hover:text-white border border-[#3D8D7A] transform hover:scale-105">
                View All
            </a>
        </div>
    </div>
    <!-- Carousel wrapper -->
    <div class="relative h-100 gap-10  flex justify-center items-center ml-20 overflow-hidden rounded-lg md:h-120">
        <!-- Item 1 -->
        <div class="hidden flex  duration-200 ease-linear" data-carousel-item>
            <div class="grid grid-cols-5 p-2  w-full ">

                <?php
                $callNewBook = $connect->query("select*from sellbook");

                while ($book = $callNewBook->fetch_array()) {
                    ?>
                    <a href="view.php?book_id=<?= $book['book_id']; ?>">
                        <div
                            ...
                            <div class=" p-2 hover:scale-105 transition  rounded  relative rounded  w-45 flex    ">
                                ...
                                <img src="images/<?= $book['sbook_img1']; ?>" alt="" class=" h-[95%] object-cover rounded w-45">
                            </div>
                            <p class="w-50 py-1 text-[#rgb(5 54 40)] truncate font-medium"><?= $book['sbook_name']; ?>
                            </p>

                            <p class="text-sm text-orange-600 font-medium text-start w-full ">by
                                <?= $book['sbook_author']; ?>
                            </p>

                            <p class="flex gap-1  justify-start w-full items-center ">
                                <span class="font-bold text-lg text-[#053628]">₹<?= $book['sbook_price']; ?>-/</span>
                                <del class="text-gray-500 text-sm "> ₹<?= $book['sbook_mrp']; ?>-/</del>
                                <span class="text-sm text-gray-800 font-medium"><span
                                        class="text-sm text-red-800 font-medium">eBook</span>:₹45-/</span>
                            </p>
                            <button class="flex items-center justify-center gap-2 w-52 py-2 mt-3 rounded-lg bg-[#3D8D7A] text-[#FBFFE4] border border-transparent 
    ...
    <button type="button"
        ...
        data-carousel-prev>
        <span
            class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-[#A3D1C6]/30 dark:bg-gray-800/30 group-hover:bg-[#FBFFE4]/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-[#3D8D7A]  dark:group-focus:ring-gray-800/70 group-focus:outline-none">
            <svg class="w-4 h-4 text-[#3D8D7A] dark:text-gray-800 rtl:rotate-180" aria-hidden="true"
