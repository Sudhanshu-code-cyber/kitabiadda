...
if (isset($_GET['bookset_id'])) {
    $bookset_id = $_GET['bookset_id'];
}

$bookset_id = $_GET['bookset_id'];

// Use prepared statement to prevent SQL injection
$bookset_query = mysqli_query($connect, "SELECT * FROM bookset_id WHERE bookset_id='$bookset_id'");
$fetch_booksets_books = mysqli_fetch_array($bookset_query);
$books_set_id = $fetch_booksets_books['books_id'];
$select_book_query = mysqli_query($connect, "SELECT * FROM books where id='$books_set_id'");
...
// $book_id = $_GET['bookset_id'] ?? null;
// $sellerdata = null;
// $sellerInfo = null;


// $getbook = $connect->query("SELECT * FROM bookset_id WHERE bookset_id = '$bookset_id'");
...
<body
    ...
    <div class="py-5 px-4  w-full md:px-10">
        <div class="flex flex-col bg-white w-full md:flex-row p-4 md:p-10  rounded shadow mt-28 book-container">
            ...
            // $sql = "SELECT set_img FROM books";  // Replace with your table & column name
            // $result = $conn->query($sql);
            ?>

            <div class="relative">
                <!-- Slider container -->
                <div id="slider" class="flex overflow-x-auto space-x-4 p-4 scroll-smooth">

                    <?php while ($img = mysqli_fetch_array($select_book_query)): ?>
                        <div class="min-w-[7rem] sm:min-w-[9rem] flex-shrink-0">
                            <img src="assets/images/<?= htmlspecialchars($img['img1']) ?>" alt="Book Cover"
                ...
                <div class="flex justify-between mt-2 px-4">
                    <button onclick="scrollSlider(-200)"
                        class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600">&larr;</button>
                    <button onclick="scrollSlider(200)"
                        class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600">&rarr;</button>
                </div>
            </div>
            ...
            <div class="w-7/12 md:w-7/12 flex flex-col gap-2 p-2 md:p-6 book-details-section">
                <div class="flex flex-col md:flex-row justify-between">
                    <div>
                        <h1 class="text-xl md:text-2xl font-semibold"><?php while ($name = mysqli_fetch_array($select_book_query)) {
                            $t_name = htmlspecialchars($name['book_name']);
                            echo "";
                        } ?></h1>
                        ...
                        <h3 class="text-base md:text-lg font-semibold book-author">Author: <span
                                class="text-[#3D8D7A]">cfghvjbkn</span></h3>
                    </div>
                ...
                <div class="flex flex-col py-5 gap-3 md:gap-5">
                    ...
                    <div class="grid grid-cols-5  md:grid-cols-5 gap-1 md:gap-2 book-highlights">
                        <div
                            ...
                            <p class="text-xs md:text-sm">cvubj</p>
                        </div>
                        <div
                            class="flex items-center border-r gap-1 border-gray-300 px-1 md:px-3 flex-col highlight-item">
                            ...
                            <p class="text-xs md:text-sm">fcghvjbkn</p>
                        </div>
                        <div
                            class="flex items-center border-r gap-1 border-gray-300 px-1 md:px-3 flex-col highlight-item">
                            ...
                            <p class="text-xs md:text-sm">v bnm,hjbknkl</p>
                        </div>
                        <div
                            class="flex items-center border-r gap-1 border-gray-300 px-1 md:px-3 flex-col highlight-item">
                        ...
                        <div
                            ...
                            <p class="text-xs md:text-sm">fgvhbjknlml</p>
                        </div>
                    </div>
